...
from models.LSBlock import *
from models.CMA_encoder import *


class GSLConv(nn.Module):
    def __init__(self, c1, c2, k=1, s=1, g=1, act=True):
        super().__init__()
        c_ = c2 // 2
        self.cv1 = Conv(c1, c_, k, s, None, g, act=act)
        self.linear = nn.Linear(c_, c_)  # Linear not conv
        self.c_ = c_
    ...
    def forward(self, x):
        ...
        x1_flat = x1.view(batch_size, channels, -1).permute(0, 2, 1)  # (batch_size, height*width, channels)
        x2_flat = self.linear(x1_flat)  # (batch_size, height*width, channels)
        x2_flat = F.normalize(x2_flat, p=2, dim=2)  # L2Norm
        x2 = x2_flat.permute(0, 2, 1).view(batch_size, channels, height, width)  # (batch_size, channels, height, width)
...
class DetectMultiBackend(nn.Module):
    ...
    def __init__(self, weights='', device=torch.device('cpu'), dnn=False, data=None, fp16=False, fuse=True):
        ...
        if pt:  # PyTorch
            ...
            raise NotImplementedError('ERROR: TF.js inference is not supported')
...
class Proto(nn.Module):
    # mask Proto module for segmentation models
